...
donutd <- dbGetQuery(con, "SELECT * from bioblitz.ak_bioblitz_taxa_by_lifeform_donut_chart_view ;")
dim(donutd)
...
p <- ggplot(donutd, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=Lifeform)) +
  ...
  geom_label_repel(aes(label = label, x = 4, y = (ymin + ymax)/2),inherit.aes = F, show.legend = F, size = 3.0) +
...
p1 <- p + scale_color_manual(name = "Lifeform",breaks = c("Coniferous Tree","Deciduous Tree","Low or Tall Shrub","Dwarf Shrub","Forb","Graminoid","Spore-bearing","Moss","Liverwort","Algae"), values = c("Coniferous Tree" = "#009E73","Deciduous Tree" = "#F0E442","Low or Tall Shrub" = "#56B4E9","Dwarf Shrub" = "#0072B2","Forb" = "#CC79A7","Graminoid" = "#7F00FF","Spore-bearing" = "#D55E00","Moss" = "#E69F00","Liverwort" = "#CC0000","Lichen" = "#999999","Fungi" = "#000000")) 
